...
define("wptored_path", WP_PLUGIN_URL . "/" . str_replace(basename( __FILE__), "", plugin_basename(__FILE__)));
define("wptored_version", "1.2");
$plugin_dir = basename(dirname(__FILE__));
$plugin = plugin_basename(__FILE__); 

define("wptored_acct_name", "wptored_admin_options");

function wptored_deactivate() {
	delete_option('wptored_seed_location');
	delete_option('wptored_acct_name');
	delete_option('wptored_user_name');
	delete_option('wptored_password');
}

function wptored_get_seed_location() {
	return get_option('wptored_seed_location');
}

function wptored_get_acct_name() {
	return get_option('wptored_acct_name');
}

function wptored_get_channel_name() {
	return get_option('wptored_channel_name');
}

function wptored_get_password() {
	return get_option('wptored_password');
}

function wptored_post($post_id) {

	$post = get_post($post_id);
	
    if (isset($_POST['wptored'])) {
		update_post_meta($post_id, 'wptored', '1');
	} 

	// if meta has been set
	if (get_post_meta($post_id, "wptored", true) === '1') {

		$user_name = wptored_get_acct_name();
		$password = wptored_get_password();
		$seed_location = wptored_get_seed_location();
		$channel = wptored_get_channel_name();
		$backlink = get_option('wptored_backlink');
...
function wptored_delete_post($post_id) {

	$post = get_post($post_id);
	
	// if meta has been set
	if ((get_post_meta($post_id, "wptored", true) == '1') || (get_post_meta($post_id, "post_from_red", true) == '1')) {
...
function wptored_delete_comment($post_id) {
...
function wptored_displayAdminContent() {
	
	$seed_url = wptored_get_seed_location();
	$password = wptored_get_password();
	$user_acct = wptored_get_acct_name();
	$channel = wptored_get_channel_name();
	$backlink = get_option('wptored_backlink');
	...
	<div class="wrap">
		...
		<form method="post" action="{$_SERVER["REQUEST_URI"]}">
			...
			Login (email): <input type="text" name="wptored_acct_name" value="{$user_acct}"/><br />
			Password: <input type="password" name="wptored_password" value="{$password}"/><br />
			Red Matrix URL: <input type="text" name="wptored_url" value="{$seed_url}"/><br />
			Optional channel nickname: <input type="text" name="wptored_channel" value="{$channel}"/><br />
			Add permalink to posts? <input type="checkbox" name="wptored_backlink" value="1" {$backlink_checked} /><br />
...
function wptored_post_checkbox() {

    add_meta_box(
        'wptored_meta_box_id', 
        'Cross Post to Red Matrix',
        'wptored_post_meta_content',
        'post',
        'normal',
        'default'
    );
...
function wptored_post_meta_content($post_id) {
    wp_nonce_field(plugin_basename( __FILE__ ), 'wptored_nonce');
    echo '<input type="checkbox" name="wptored" value="1" /> Cross post?';
}

function wptored_post_field_data($post_id) {
    ...
    // security check
	if((! array_key_exists('wptored_nonce', $_POST))
    || (!wp_verify_nonce( $_POST['wptored_nonce'], plugin_basename( __FILE__ ))))
        return;
...
function wptored_display_admin_page() {
	
	if ((isset($_REQUEST["wptored_acct_name"])) && (isset($_REQUEST["wptored_password"]))) {
		
		$password = $_REQUEST["wptored_password"];
		$red_url = $_REQUEST["wptored_url"];
		$channelname = $_REQUEST['wptored_channel'];

		
		update_option('wptored_acct_name', $_REQUEST["wptored_acct_name"]);
		update_option('wptored_channel_name', $channelname);
		update_option('wptored_seed_location', $red_url);
		update_option('wptored_password', $password);
		update_option('wptored_backlink', $_REQUEST['wptored_backlink']);		
	}
	
	wptored_displayAdminContent();
}

function wptored_settings_link($links) { 
	$settings_link = '<a href="options-general.php?page=xpost-to-redmatrix">Settings</a>'; 
  	array_unshift($links, $settings_link); 
  	return $links; 
}

function wptored_admin() {
	add_options_page("Crosspost to redmatrix", "Crosspost to redmatrix", "manage_options", "xpost-to-redmatrix", "wptored_display_admin_page");
}

register_deactivation_hook( __FILE__, 'wptored_deactivate' );

add_filter("plugin_action_links_$plugin", "wptored_settings_link");

add_action("admin_menu", "wptored_admin");
add_action('publish_post', 'wptored_post');
add_action('add_meta_boxes', 'wptored_post_checkbox');
add_action('save_post', 'wptored_post_field_data');
add_action('before_delete_post', 'wptored_delete_post');

add_action('delete_comment', 'wptored_delete_comment');

add_filter('xmlrpc_methods', 'red_xmlrpc_methods');

add_filter('get_avatar', 'wptored_get_avatar',10,5);
...
function wptored_get_avatar($avatar,$id_or_email,$size,$default,$alt) {
